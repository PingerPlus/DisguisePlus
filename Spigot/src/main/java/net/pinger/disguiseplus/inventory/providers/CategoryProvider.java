...
import net.pinger.disguiseplus.prompts.packs.CreatePackPrompt;
...
public class CategoryProvider implements InventoryProvider {
    ...
    public void update(Player player, InventoryContents contents) {
        ...
        // Get the skins
        List<? extends net.pinger.disguiseplus.SkinPack> packs = factory.getSkinPacks(this.category);
        ClickableItem[] items = new ClickableItem[packs.size()];

        for (int i = 0; i < items.length; i++) {
            // Get the pack
            net.pinger.disguiseplus.SkinPack pack = packs.get(i);
